...
namespace Visit_Syria
{
    ...
    public partial class _Default : System.Web.UI.Page
    {
        ...
        protected void Button3_Click(object sender, EventArgs e)
        {
            ...
            string check = (@"select password from [user] where username='" + user + "'");
            SqlCommand com = new SqlCommand(check, conn);
            string checkr= recievedpass.Text.GetHashCode().ToString();
            //int temp = Convert.ToInt32(com.ExecuteScalar().ToString());
            conn.Close();
            if (checkr == recievedpass.Text.GetHashCode().ToString())
            {
                try
                {
                    conn.Open();
        ...
        protected void Button2_Click(object sender, EventArgs e)
        {
            ...
            {
                conn.Open();
                string checkemail = (@"select Gmail from [user] where username='" + TextBox1.Text + "'");
                SqlCommand pass = new SqlCommand(checkemail, conn);

                string checkans1 = (@"select firstans from [user] where username='" + TextBox1.Text + "'");
                SqlCommand pass1 = new SqlCommand(checkans1, conn);

                string checkans2 = (@"select secondans from [user] where username='" + TextBox1.Text + "'");
            ...
            if (temp == 1 && ans1 == TextBox2.Text && ans2 == TextBox3.Text)
            {
